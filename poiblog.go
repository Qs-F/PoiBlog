...
func (b *Blog) ReadConf(file string) {
	raw, err := os.Open(file)
	check(err)
	conf := json.NewDecoder(raw)
	err = conf.Decode(&b)
	check(err)
}

func (b *Blog) NewServer() {
	http.Handle("/box-design/", http.StripPrefix("/box-design/", http.FileServer(http.Dir(b.RootDir+"/box-design"))))
	http.Handle("/img/", http.StripPrefix("/img/", http.FileServer(http.Dir(b.RootDir+"/img"))))
	http.HandleFunc("/article", b.articleHandler)
	http.HandleFunc("/", b.mainHandler)
	err := http.ListenAndServe(":"+strconv.Itoa(b.Port), nil)
	check(err)
}

func (b *Blog) mainHandler(w http.ResponseWriter, r *http.Request) {
	articles, err := ioutil.ReadDir(b.RootDir + "/articles/")
	w.Header().Set("Content-Type", "text/html")
	re := regexp.MustCompile("404|.git|([\\s\\S]*)\\.md")
	var result ByArticles
	var tmp, output string
	topTemplate, err := ioutil.ReadFile(b.RootDir + "/tmpl/top.html")
	check(err)
	if err != nil {
...
func (b *Blog) articleHandler(w http.ResponseWriter, r *http.Request) {
	...
	_, err := os.Stat(b.RootDir + "/articles/" + string(articleName[0]) + ".md")
	if err != nil {
		articleName = []string{"404"}
	}
	md, err := ioutil.ReadFile(b.RootDir + "/articles/" + string(articleName[0]) + ".md")
	var date string
	f := func() {
		tmp, _ := os.Lstat(b.RootDir + "/articles/" + articleName[0] + ".md")
		date = tmp.ModTime().Format("2006/01/02 15:04")
	}
	f()
	check(err)
	articleTemplate, err := ioutil.ReadFile(b.RootDir + "/tmpl/article.html")
	check(err)
